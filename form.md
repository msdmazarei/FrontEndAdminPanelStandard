# Forms
All attributes mentioned below, could/should be passed to components as **props**.  
All Items cross referenced between mdn and w3c specs.

## Events
* **form** tag emits only 2 events:
    * reset
    * submit
* standard form elements emit only 2 events:
    * change
    * input
* there is some other events that could be passed to form or other intended parent component, based on the functionality expected from form element.

## Form Tag
Each Form tag has some explicit *attributes* which should be defined on creation stage.
